...
void outputDirectory(DirectoryEntry *root, uint16_t size){
    ...
    for(int i = 0;i<size; i++){
        if(checkFileName(root[i].DIR_Name) == 1){
            break;
        }else if(checkFile(root[i].DIR_Attr) != 1){
...
//Allows for full path or short part access to files as long as the short file name used with no spaces or dots between name and type.
int main(){
    int quit = 1;
    ...
    while (quit == 1){
        char text[80];
        printf("Please enter the file you want to read:(use short name no spaces of dot)");
        scanf("%s",text);
