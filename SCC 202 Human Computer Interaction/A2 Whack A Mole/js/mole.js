...
const positionMole = function () {
  ...
  const normalizedPos = getMolePosition(scale);
...
const updateMoleParams = function (
  visible,
  clickable,
  transparent,
  tinge = "white"
  ...
  try {
    if (tinge == "white") {
      //setting color on a-image isn't trivial apparently
    ...
    console.error("ERROR: Chrome-related bug triggered", e.type);
    return false;
  }
  return true;
};
