...
const initGame = function () {
  ...
  console.info("INFO: Game initing...");
  gameState.pid = Math.floor(Math.random() * 900000) + 1000000;
  gameState.gameStart = Date.now();
  let firstUpdate = updateMoleParams(false, false, true); //not visible, not clickable, and transparent,
  if(firstUpdate===false){//weird chrome bug
    console.info("INFO: Weird Chromebug encountered. Standby... retrying...");
  ...
  document
   ...
   console.info("INFO: Game init complete...");
};

const readyToStartTrial = function () {
  gameState.trialState = "waitStartClick";
  gameState.trialCnt++;
  document.querySelector('#round').textContent = "Round: "+(gameState.trialCnt+1);
...
const userClickedMole = function () {
  ...
  gameState.score++;//score incremented by 1
  document.querySelector("#score").textContent = "Score: " + gameState.score; //update user about change in score
  gameState.trialState = "trialEnded";
